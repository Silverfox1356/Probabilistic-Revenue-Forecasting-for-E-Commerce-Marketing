...
**Forecasting pipeline.** `load_data` (three-source schema adapter) → `validate` (consistency checks) → `run_forecast`, which filters to active campaigns and, per entity at each aggregation level, computes a recent-window revenue level, detrended day-of-week/month seasonal factors, bootstrap-simulated P10/P50/P90 revenue, elasticity-based budget response, and ROAS ranges → aggregate blended metrics → JSON.
